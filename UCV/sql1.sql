...
'ACE10' AS codigo_producto, 'Acetaminofen Sol. 60mL' AS producto#(lf)    UNION ALL#(lf) SELECT #(lf)        
...
#(lf) SELECT #(lf)    p.codigo, #(lf)    p.cliente, #(lf) 
    ...
    WHEN p.codigo_producto = 'CLA3' THEN 'CLA7'#(lf) 
    ...
    WHEN p.codigo_producto = 'ACE6' THEN 'ACE10'#(lf) 
    ...
    p.factura,#(lf) 
    COALESCE(sc.producto, p.producto) AS producto#(lf)
    FROM [iScalaDB].[dbo].[powerBI] p#(lf)LEFT JOIN SKU_Correctos sc ON #(lf) 
    CASE #(lf) 
    WHEN p.codigo_producto = 'API1' THEN 'API3'#(lf) 
    WHEN p.codigo_producto = 'CLA3' THEN 'CLA7'#(lf) 
    WHEN p.codigo_producto = 'DES4' THEN 'DES3'#(lf) 
    ...
    p.codigo <> '' AND#(lf) 
    p.codigo_producto NOT LIKE '%MM'#(lf)ORDER BY 
    p.fecha DESC;#(lf)"]),